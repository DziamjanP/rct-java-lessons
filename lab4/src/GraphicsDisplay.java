...
public class GraphicsDisplay extends JPanel {
    ...
    private void paintLabels(final Graphics2D canvas) {
        ...
        if (selectedMarker >= 0) {
            ...
            String label = "X = " + GraphicsDisplay.formatter.format(graphicsData[selectedMarker][0]) + ", Y = " + GraphicsDisplay.formatter.format(graphicsData[selectedMarker][1]);
    ...
    protected void paintArea(Graphics2D canvas) {
        ...
        for (; i < graphicsData.length - 1; i++) {
            if (xyToPoint(graphicsData[i][0], graphicsData[i][1]).y > mainY) { break; }
        }
